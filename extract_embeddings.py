...
for (i, imagePath) in enumerate(imagePaths):
	...
	if len(detections) > 0:
		...
		if confidence > 0.5:
			...
			#cv2.imwrite("test/face_test.jpg", face) #God test to quickly see what minus 25% of face looks like

			# ensure the face width and height are sufficiently large
			if fW < 20 or fH < 20:
				continue
			else:
                                cv2.imwrite("god_periocular_training_data_generation_test/face_test__%d.jpg" % total, face)
