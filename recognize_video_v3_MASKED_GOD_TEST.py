...
INPUT_FILE_NAME = "_test/bennett_god_clip_masked.mp4"
...
while True:
	...
	for i in range(0, detections.shape[2]):
		...
		if confidence > 0.5:
			...
			if ( DISCOVERY_COUNT < 2 ):
                                cv2.imwrite("overview_recognized/Recognized_Masked_Suspect=%s__videoStamp=%d_seconds.jpg" % (name, cv2.CAP_PROP_FRAME_COUNT), face)
	...
	cv2.imshow("God's ai project is now processing : %s" % INPUT_FILE_NAME, frame)
